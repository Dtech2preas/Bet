...
const quotationsManager = {
    ...
    renderTable() {
        ...
        this.quotations.forEach(q => {
            ...
            html += `
                <tr>
                    <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">${q.reference}</td>
                    <td class="px-6 py-4 text-sm text-gray-500">${client ? client.companyName : 'Unknown'}</td>
                    <td class="px-6 py-4 text-sm font-semibold">R ${q.total}</td>
                    <td class="px-6 py-4"><span class="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-800">${q.status}</span></td>
